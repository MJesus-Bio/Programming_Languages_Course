...
;   (check-equal? (list-nth-mod (list 0 1 2 3 4) -2) (error "list-nth-mod: negative number"))
;   (check-equal? (list-nth-mod null 2) (error "list-nth-mod: empty list"))
   (check-equal? (list-nth-mod (list 0 1 2 3 4) 7) 2)
   
   ; stream-for-n-steps test
   (check-equal? (stream-for-n-steps ones 2) (list 1 1) "stream-for-n-steps test")
   (check-equal? (stream-for-n-steps counter 4) (list 1 2 3 4))
   ...
   (check-equal? (stream-for-n-steps (stream-add-zero counter) 3) (list (cons 0 1) (cons 0 2) (cons 0 3)))
   (check-equal? (stream-for-n-steps (stream-add-zero dan-then-dog) 3) (list (cons 0 "dan.jpg")
                                                                             (cons 0 "dog.jpg")
                                                                             (cons 0 "dan.jpg")))
   ...
   (check-equal? (vector-assoc 4 (vector (cons 2 1) (cons 3 1) 4 (cons 4 1))) (cons 4 1))
   (check-equal? (vector-assoc 4 (vector (cons 2 1) (cons 3 1) 4 (cons 5 1))) #f)
   (check-equal? (vector-assoc 4 (vector (cons 2 1) (cons 3 1) 4 (cons 1 4))) #f)
   (check-equal? (vector-assoc "b" (vector (cons "a" "a") (cons "b" "a"))) (cons "b" "a"))
   ...
   (check-equal? (while-less 7 do (begin (set! a (+ a 1)) (print "t") a)) #t)
